...
const mediaLibrary = [
  ...
  h("Image alt text", HeadingLevel.HEADING_2),
  p(
    "Every image field on a document has an ",
    bold("Alt text"),
    " or ",
    bold("Image description"),
    " sub-field. Fill this in — it's what screen readers read out, and it's how search engines understand what the image is. One short sentence is enough: \"Bobby and Scott on site at Carlton Gardens.\"",
  ),
...
const homePage = [
  ...
  field("Hero video URL", "Direct link to the looping video file (.mp4)."),
  field("Hero statement", "The big editorial line that opens the site."),
  field("Hero scroll label", "Text near the bottom prompting scroll. Default \"Scroll down\"."),
  field("Hero CTA", "Label + href for the Project button under the statement."),
  ...
  field("Heading", "Display heading."),
  field("CTA", "Link to /projects (full archive)."),
  p("The actual project cards pull from the Projects collection — toggle ", bold("Featured"), " on individual projects to choose which 5–6 surface here."),
...
const aboutPage = [
  ...
  h("Hero", HeadingLevel.HEADING_3),
  field("Hero title", "The big display heading. Use \\n in the field for line breaks."),
  field("Hero CTA", "Optional button — typically anchors down to the team section."),
  field("Hero CTA href", "e.g. #team to scroll to the team grid."),
  field("Centre image", "The portrait shown beside the heading on desktop. On mobile this is the only image shown."),
  field("Left image / Right image", "Used by the desktop 3-image scroll-driven hero."),

  h("Intro", HeadingLevel.HEADING_3),
  field("Section heading", "Display heading."),
...
const servicesPage = [
  ...
  field("Hero CTA", "Label + href + page name."),
  field("Centre / Left / Right images", "Hero image set — same pattern as About / Careers / Sustainability."),
  h("Intro", HeadingLevel.HEADING_3),
  field("Heading", "Display heading."),
  field("Body", "Multi-paragraph."),
  h("Services list", HeadingLevel.HEADING_3),
  field("Section label", "Eyebrow."),
  field("Heading", "Display heading."),
  field("Items", "Each service — title + body. Drag to reorder."),
  ...
  field("Section label", "Eyebrow."),
  field("Heading", "Display heading."),
  field("Items", "Stat cards — value + label + optional footnote, same shape as the home page Stats section."),
...
const sustainabilityPage = [
  ...
  field("Hero title", "Display heading."),
  field("Hero CTA label / href / page name", ""),
  field("Hero left / centre / right images", "Three-image hero."),
  h("Intro", HeadingLevel.HEADING_3),
  field("Heading", ""),
  field("Body", "Multi-paragraph."),
  h("Stats (mobile fade / desktop sticky)", HeadingLevel.HEADING_3),
  field("Section label", "Eyebrow above the staircase."),
  field("Heading", "Display heading."),
  field("Items", "Up to 4 stat cards (value + label + footnote). Extras are ignored."),
  h("Feature image", HeadingLevel.HEADING_3),
  field("Section label", "Eyebrow."),
  field("Heading", "Display heading."),
  field("Body", "Long-form copy."),
  field("Image", "Large editorial photograph."),
  field("CTA label / link / page name", "Default link points to /services."),
  h("Legacy projects", HeadingLevel.HEADING_3),
  field("Section label", "Default \"Recent work\"."),
  field("Items", "Three projects shown as image + name + year."),
  h("Commitment", HeadingLevel.HEADING_3),
  field("Heading / Body", ""),
  h("Principles", HeadingLevel.HEADING_3),
  field("Items", "Each — title + body. The principles list."),
  h("Certifications", HeadingLevel.HEADING_3),
  field("Section label", "Default \"Frameworks we follow\"."),
  field("Items", "Standards / frameworks shown as a wrapping list — e.g. BREEAM, NABERS UK, RICS Whole Life Carbon."),
  h("SEO", HeadingLevel.HEADING_3),
  field("Browser tab title", ""),
  field("Meta description", ""),
];
...
const collections = [
  ...
  field("Category", "Reference to a Project Category. Used as a filter chip. Required."),
  field("Short description", "1–2 sentences shown on the listing card and below the title on the project page."),
  field("Location", "City + region. e.g. \"London, UK\"."),
  field("Year", "Year completed."),
  field("Tags", "Searchable tags. Optional."),
  field("Featured", "Toggle on to surface this project on the home page's Featured projects strip."),
  h("Images", HeadingLevel.HEADING_3),
  field("Featured image", "The cover image — used on the listing, the project hero, and the lightbox first slide. Use \"Pick from library\" and pick by tag."),
  field("Additional images", "Gallery images. Drag to reorder; the first slide of the lightbox is the featured image, then these in order."),
  h("Project Stats", HeadingLevel.HEADING_3),
  field("Expertise", "References to Expertise documents. Lists the disciplines used on this project."),
  field("Team", "References to Team Member documents. Shows their tiles in the Project team grid."),
  field("Stats", "Custom stat rows beyond Location and Year (which are auto-prepended). Each — label + value. e.g. \"Sq ft\" / \"42,000\"."),
  ...
  h("Testimonials", HeadingLevel.HEADING_3),
  field("Testimonials section", "Optional — pick testimonials to show in a band on the project page. Leave empty to hide the section."),

  pageBreak(),

  h("Project Categories", HeadingLevel.HEADING_2),
  p("Used to filter / group projects. Keep this list small — too many makes the filter chips overwhelming."),
  field("Title", "Display name, e.g. \"Hospitality\"."),
  field("Slug", "URL-friendly identifier."),
  field("Order", "Number controlling the chip order on the projects page. Lower = earlier."),

  h("Expertise", HeadingLevel.HEADING_2),
  p("Disciplines listed on each project's project-detail page, e.g. Spatial planning, M&E coordination."),
  field("Title", "Display name."),
  field("Slug", "URL-friendly identifier."),
  field("Order", "Sort order across the system."),

  h("Team Members", HeadingLevel.HEADING_2),
  p("Everyone on the About page team grid + each project's team grid."),
  field("Name", "Full name. Required."),
  field("Role", "Job title."),
  field("Category", "The category slug — must match one in the About Page → Team categories list (e.g. \"leadership\", \"site-team\"). This controls which group they appear under on the About page."),
  field("Image", "Portrait — square crop reads best."),
  field("LinkedIn URL", "Optional. If filled, a small LinkedIn icon appears on their tile."),
  field("Order", "Sort order within their category."),

  h("Partners", HeadingLevel.HEADING_2),
  p("Logos shown in the marquee at the foot of every page."),
  field("Name", "Used as the partner's accessible label on the marquee."),
  field("Slug", "URL-friendly identifier."),
  field("Logo", "SVG file — uploads as a file, not an image. Use a single-colour SVG (it's coloured by the theme on hover). Keep filenames simple — partner-name.svg."),
  field("Order", "Position in the marquee. Lower = earlier."),

  h("Testimonials", HeadingLevel.HEADING_2),
  field("Quote", "The actual testimonial — keep it punchy. Long quotes wrap to many lines on mobile."),
  field("Author", "The person who said it."),
  field("Title", "Their job title at the time."),
  field("Partner", "Reference to a Partner document — their company logo + name. Optional but recommended."),
  field("Featured", "Toggle on to use this testimonial in the home page testimonials slider."),
