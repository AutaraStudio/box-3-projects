/**
 * Legal Page Seed Content
 * =======================
 * GDPR-aligned baseline copy for the Privacy Policy and Terms &
 * Conditions pages. The client should review and amend with their
 * solicitor before relying on this in production — it is a sensible
 * starting point, not legal advice.
 *
 * Both documents are shaped as `legalPage` Sanity docs with
 * portable-text section bodies.
 */
...
const REGISTERED_ADDRESS = "Studio 4, 12 Fitzroy Square, London, W1T 6EQ";
...
/** Bullet list — each string becomes an <li> */
...
/** Paragraph with one inline link. `[text](href)` style — but as
 *  portable-text, the link is split out as a marked span. */
function pWithLink(
  before: string,
  linkText: string,
  href: string,
...
function buildLegalDoc(args: {
  id: string;
  slug: string;
  title: string;
  intro: string;
  metaDescription: string;
  sections: Array<{ heading: string; anchor: string; body: Block[] }>;
}) {
  return {
    _id: args.id,
    ...
    metaTitle: `${args.title} — ${TRADING_NAME}`,
